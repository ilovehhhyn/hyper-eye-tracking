#!/usr/bin/env python3
"""
Computer A - FIXED Synchronized Eye Gaze Sharing (Server)
...
class RobustSyncServer:
    ...
    def start_server(self):
        """Start the synchronization server with robust settings"""
    ...
    def send_message(self, message_type, data=None, retry_count=3):
        ...
        for attempt in range(retry_count):
            try:
                ...
                print(f"A: Sent {message_type} (attempt {attempt + 1})")
                return True
            except Exception as e:
                print(f"A: Send error (attempt {attempt + 1}): {e}")
                time.sleep(0.1)
        return False
...
def update_remote_gaze_display():
    ...
    if remote_gaze_data.get('valid', False):
        try:
            ...
            pass
# Computer A (Server) - Memory Game Experiment
# IP: 100.1.1.10
...
#!/usr/bin/env python3
"""
Computer A (Server) - Modified functions and run_synchronized_experiment
...
def load_conditions():
    ...
    try:
        with open('dyad_conditions.json', 'r') as f:
            data = json.load(f)
            conditions = {
                'easy': data['top_layouts_array_fixed_4']  # 4-element arrays for 2x2 grids
            }
        print("✓ Conditions loaded from dyad_conditions.json (easy difficulty - 2x2 grids)")
    except FileNotFoundError:
        print("dyad_conditions.json not found. Using default easy conditions.")
        # Default 4-element condition if file not found
        conditions = {
            'easy': [[0, 1, 2, 3], [1, 0, 3, 2], [2, 3, 0, 1], [3, 2, 1, 0]]  # 4-element arrays
        }
    except Exception as e:
        print(f"Error loading conditions: {e}")
        conditions = {
            'easy': [[0, 1, 2, 3], [1, 0, 3, 2], [2, 3, 0, 1], [3, 2, 1, 0]]
        }
...
def create_synchronized_grid(condition_array, seed):
    """Create 8x8 physical grid from 4-element condition array (2x2 logical -> 8x8 physical)"""
    ...
    # Grid setup - 2x2 logical grid becomes 8x8 physical grid (each logical cell = 4x4 physical block)
    ...
    # Each position in 2x2 logical grid becomes a 4x4 block in 8x8 physical grid
    for logical_row in range(2):
        for logical_col in range(2):
            category = condition_categories[logical_row * 2 + logical_col]
            selected_image_idx = selected_images[category]
           
            # Fill 4x4 block with this image/category
            for block_row in range(4):
                for block_col in range(4):
                    # Calculate physical position
                    physical_row = logical_row * 4 + block_row
                    physical_col = logical_col * 4 + block_col
                    
                    x_pos = start_x + physical_col * grid_spacing
                    y_pos = start_y - physical_row * grid_spacing
    ...
    print(f"✓ Synchronized 2x2 logical grid (8x8 physical) created with seed {seed}")
  
def select_target_position(seed):
    """Select target position deterministically using seed"""
    random.seed(seed + 1000)  # Different seed offset for target selection
    return random.randint(0, 63)  # 8x8 = 64 positions
...
def run_synchronized_experiment():
    ...
    sync_server = RobustSyncServer(LOCAL_IP, REMOTE_IP, SYNC_PORT)
    sync_server.start_server()
   
    # Wait for client connection
    print("A: Waiting for Computer B to connect...")
    connected = False
    timeout_start = time.time()
   
    while not connected and (time.time() - timeout_start) < 60:
        ...
        # Show waiting screen with gaze (but no sharing yet)
        win.clearBuffer()
        status_text.setText(f"Waiting for Computer B... {int(time.time() - timeout_start)}s")
        status_text.draw()
        win.flip()
    ...
    print("A: Starting synchronized experiment")
    ...
    for trial_num in range(total_trials):
        ...
        print(f"\n=== A: TRIAL {current_trial} ===")
       
        # Generate trial parameters (Server A controls randomization)
        trial_seed = random.randint(1, 1000000)
        available_conditions = conditions['easy']
        condition_index = random.randint(0, len(available_conditions) - 1)
        selected_condition = available_conditions[condition_index]
        target_position = select_target_position(trial_seed)
       
        # Create grid to determine correct answer
        create_synchronized_grid(selected_condition, trial_seed)
        correct_category = grid_stimuli[target_position]['category']
       
        # ========== STAGE 1: GRID DISPLAY ==========
        print("A: Stage 1 - Grid Display")
        GAZE_SHARING_ACTIVE = True  # ACTIVATE gaze sharing
       
        # Send stage signal with all trial parameters
        sync_server.send_message('stage_grid_display', {
            'trial_number': current_trial,
            'stage': 'grid_display',
            'seed': trial_seed,
            'condition_index': condition_index,
            'condition_array': selected_condition,
            'target_position': target_position,
            'target_category': correct_category
        })
...
# 3. MODIFY STAGE 2 in run_synchronized_experiment() (replace the entire Stage 2 section)
        ...
        GAZE_SHARING_ACTIVE = False  # ACTIVATE gaze sharing
...
#            update_remote_gaze_display()
            ...
            resp_msg = sync_server.wait_for_response('response_update',timeout=0.1)#get_message(timeout=0.1)
        ...
        # ========== STAGE 3: FEEDBACK ==========
        print("A: Stage 3 - Feedback")
        GAZE_SHARING_ACTIVE = False  # ACTIVATE gaze sharing
       
        # Evaluate first response
        trial_score = 1 if evaluate_response(first_response, correct_category) else 0
        total_score += trial_score
       
        sync_server.send_message('stage_feedback', {
            'trial_number': current_trial,
...
#        print(core.getTime())
        ...
        print(f"A: Trial {current_trial} completed. Score: {trial_score}")
    ...
    filename = f'sync_server_A_{time.strftime("%Y%m%d_%H%M%S")}.csv'
    df.to_csv(filename, index=False)
   
    final_score_pct = (total_score / total_trials) * 100
    print(f"A: Experiment complete! Score: {total_score}/{total_trials} ({final_score_pct:.1f}%)")
